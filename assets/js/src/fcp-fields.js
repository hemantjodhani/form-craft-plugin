...
function fcp_field_appender_admin() {
    ...
    $(document).on('click', '.fcp-single-widget', function () {
        ...
        if (field_name === "text") {
            ...
            $('.fcp-form-fields-wrapper').append(`<div class="fcp-single-field-wrap" data-id=${fcp_field_id} >${MINI_SETTINGS_BAR}<div><label class="fcp-label-admin">Password</label></div><input class='fcp-form-control' type="${field_name}" disabled></div>`);
